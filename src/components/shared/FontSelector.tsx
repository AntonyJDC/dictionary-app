...
export default function FontSelector() {
  ...
  return (
    <div className="relative">
      <button
        className="py-2 flex items-center gap-x-4 rounded-md"
      ...
      {open && (
        <ul className="absolute mt-2 w-32 shadow-md rounded-md bg-accent z-20">
          {fonts.map((f) => (
            <li
              key={f.value}
              className="cursor-pointer px-4 py-2 hover:font-bold hover:bg-[#A445ED]/50"
              onClick={() => {
